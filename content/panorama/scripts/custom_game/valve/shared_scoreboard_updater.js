...
(function()
{
	GameEvents.Subscribe( "UpdateScore", UpdateScore);
})();

function UpdateScore(keys)
{
	var teamID = keys.teamID;
	var score = keys.score;
	
	var teamPanel = $( "#MultiteamScoreboard" ).FindChild("_dynamic_team_" + teamID);
	_ScoreboardUpdater_SetTextSafe( teamPanel, "TeamScore", score );
	$.Msg("Setting team score. Team: " + teamID + ", score: " + score);
}
...
//		$.Msg( "UpdateTeamPanel.Create: ", teamPanelName, " = ", scoreboardConfig.teamXmlName );
		...
		teamPanel.AddClass("Team" + teams[teamId - 2])
