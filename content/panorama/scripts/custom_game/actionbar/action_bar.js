...
function UpdateAbilityList()
{
	...
	for ( var i = 0; i < Entities.GetAbilityCount( queryUnit ); ++i )
	{
		...
		// if it is not a controller ability do nothing
		if (Abilities.GetAbilityName(ability).slice(0,14) != "hlw_controller")
		{
			continue;
		}
		...
		if ( nUsedPanels >= m_AbilityPanels.length )
		{
			...
			$.Msg("[HLW-js] DisplayedStatus = ", Abilities.IsDisplayedAbility(ability));
			
			// create a new panel
			var abilityPanel = $.CreatePanel( "Panel", abilityListPanel, "" );
			abilityPanel.BLoadLayout( "file://{resources}/layout/custom_game/hlw_actionbar_ability.xml", false, false );
