...
function CreateFissure(keys)
	...
	local initialization_time = ability:GetLevelSpecialValueFor("initialization_time", (ability:GetLevel() -1))
	...
	-- Add Sound, update abilities current_distance, and stun units
	local time = 0
	local position = startPos
	local total_procs = 7
	local previous_position = startPos
	local distance_step = (startPos - endPos):Length2D() / total_procs
	local distance = distance_step
	ability.current_distance = distance
	local time_step = initialization_time / total_procs
	local enemy_units
	local all_units

	Timers:CreateTimer(0, function()
			EmitSoundOnLocationWithCaster(position, "Hero_EarthShaker.Fissure", caster)
			position = position + (direction * distance_step)
			distance = distance + distance_step
			ability.current_distance = distance
			time = time + time_step

			-- Units to be stunned and damaged by the fissure
			enemy_units = FindUnitsInLine(caster:GetTeam(), previous_position, position, nil, fissure_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0)
			
			-- Units to be moved by the fissure
			all_units = FindUnitsInLine(caster:GetTeam(), previous_position, position, nil, width, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0)

			-- Loops through the targets to be damaged/stunned
			for _, unit in ipairs(enemy_units) do
				if unit:HasModifier("modifier_stunned") ~= true then
			...
			-- Loops through the units to be moved
			...
			if time >= initialization_time then
				return
			end
			return time_step
		end
...
function CheckPosition(keys)
	...
	for i,unit in ipairs(units) do
		...
		if target == unit then
			...
			local target_distance = ability.current_distance
			local target_direction = (target_vector_distance):Normalized()
